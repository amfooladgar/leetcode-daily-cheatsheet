...
8. Produce clear, idiomatic Python 3 code:
   ...
   - Do not optimize code at the expense of explaining the idea in the
     surrounding fields.
   - Recursion-depth safety: if your solution recurses and the recursion
     depth can grow with the input size (e.g. a DFS over a tree, linked
     list, or graph whose worst-case shape is fully skewed / degenerate),
     and the problem's stated constraints allow that depth to reach or
     exceed ~1000, defend against `RecursionError` explicitly. Either add
     `import sys` and a single `sys.setrecursionlimit(...)` call near the
     top of the module (sized to the constraint's upper bound plus a
     safe margin, e.g. `sys.setrecursionlimit(10**6)`), or write the
     traversal iteratively with an explicit stack. CPython's default
     limit is 1000 and the test harness consumes some of that budget, so
     an unguarded recursion whose depth can reach n = 1000 is a real
     crash on a legal input, not a hypothetical. If you raise the limit,
     say so in one clause of the `correctness` or `complexity.explanation`
     field, and keep the stated space complexity in terms of the actual
     stack depth (h, or n for a skewed shape).
